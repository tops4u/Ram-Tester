      ...
      for (uint8_t pb_i = 0; pb_i < 8; pb_i++) {
        ...
        for (uint8_t pd_i = 0; pd_i < 16; pd_i++) {
          ...
          if ((PINC ^ exp) & 0x04) {
            ...
            // 4532 half detection: A7 = PD6, upper col half when set
            bool is_upper = (PORTD & 0x40) != 0;

            if (type == T_4164) {
              if (chip_half_status == 0) {
                chip_half_status = is_upper ? -1 : 1;
              } else if ((chip_half_status == -1 && !is_upper) || (chip_half_status == 1 && is_upper)) {
                error(patNr + 1, 2);
              }
              return;  // Bad half error — continue with next pattern
            }
            if (type == T_4532_L && is_upper) return;
            if (type == T_4532_H && !is_upper) return;
            error(patNr + 1, 2);
            return;
          }
        }
 ...
 *
 * 4532 Detection: tracked across all 6 patterns via chip_half_status.
 * Reclassification happens after all patterns complete.
 */
static void run_16Pin_tests() {
  uint16_t total_rows = ramTypes[type].rows;
  ...
  if (ramTypes[type].flags & RAM_FLAG_NIBBLE_MODE) total_rows /= 2;  // for Nibble Mode we use Nibble Mode for PatNr 4 & 5 ommiting A8 manipulation as this is used for nibble bit selection
  ...
  // chip_half_status: -1 = errors in upper half (lower good), +1 = errors in lower half (upper good)
  if (type == T_4164 && chip_half_status != 0) {
    if (chip_half_status == -1) {
      type = T_4532_L;  // Lower column half functional
    } else {
      type = T_4532_H;  // Upper column half functional
    }
    writeRAMType((const __FlashStringHelper *)ramTypes[type].name);
  }
}
...
void __attribute__((hot)) writeRow_16Pin(uint16_t row, uint16_t cols, uint8_t patNr) {
  ...
  if (!is_nibble) {
    // ====== NORMAL PATH (4164/41256/4816): Full-row page mode, NO RAS cycling ======
    // One RAS cycle for ALL columns — proven working in 4.0.1
    rasHandling_16Pin(row);
    WE_LOW16;
    cli();
  ...
  if (error_found) {
    if (type == T_4164) {
      ...
      if (chip_half_status == 0) {
        ...
        if ((chip_half_status == -1 && !is_upper_col_half) || (chip_half_status == 1 && is_upper_col_half)) {
          error(patNr + 1, check);
          return;
        }
      }
