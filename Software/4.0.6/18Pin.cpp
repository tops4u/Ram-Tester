...
// Column contributions for index 0-7: maps to A9(bit9), A8(bit8), A7(bit7)
static const uint16_t pb_col_18Alt[8] PROGMEM = {
  0x000, 0x200, 0x100, 0x300, 0x080, 0x280, 0x180, 0x380
};

/**
 * Address Setter using Split LUTs (not inlined — no longer in hot inner loops)
 */
void setAddr_18Pin_Alt(uint16_t a) {
  if (a & 1) SBI(PORTC, 4);
  else CBI(PORTC, 4);

  uint8_t val_d = pgm_read_byte(&lut_18Pin_Low[a & 0x7F]);
...
static void fastPatternTest_18Pin_Alt(uint8_t patNr);
...
void test_18Pin() {
  ...
  if (type == T_411000) {
    ...
    // Patterns 0-3: fast port-ordered write-all then read-all
    for (uint8_t patNr = 0; patNr < 4; patNr++) {
      fastPatternTest_18Pin_Alt(patNr);
    }
    // Patterns 4-5: pseudo-random with retention testing
 ...
 * @note error() requires interrupts enabled (sei()) for delay() to function
 */
...
/**
 * Reconstruct logical column address from current port register values
 *
 * Only called in error path — marked noinline to keep hot loops small.
 * Reverses the CALC_PORTD / CALC_PORTB mapping:
 *   PD0→A1, PD1→A2, PD2→A3, PD5→A4, PD6→A5, PD7→A6
 *   PB4→A7, PB2→A8, PB0→A9
 *   PC4→A0
 *
 * @param pd Current PORTD value
 * @param pb Current PORTB value
 * @param pc Current PORTC value (only bit 4 = A0)
 * @return Logical 10-bit column address
 */
static uint16_t __attribute__((noinline)) reconstructCol_18Pin_Alt(uint8_t pd, uint8_t pb, uint8_t pc) {
  uint16_t col = 0;
  if (pc & 0x10) col |= 0x001;   // PC4 → A0
  if (pd & 0x01) col |= 0x002;   // PD0 → A1
  if (pd & 0x02) col |= 0x004;   // PD1 → A2
  if (pd & 0x04) col |= 0x008;   // PD2 → A3
  if (pd & 0x20) col |= 0x010;   // PD5 → A4
  if (pd & 0x40) col |= 0x020;   // PD6 → A5
  if (pd & 0x80) col |= 0x040;   // PD7 → A6
  if (pb & 0x10) col |= 0x080;   // PB4 → A7
  if (pb & 0x04) col |= 0x100;   // PB2 → A8
  if (pb & 0x01) col |= 0x200;   // PB0 → A9
  return col;
}

/**
 * Fast pattern test for 411000 patterns 0-3 using port-ordered column iteration
 *
 * Instead of calling setAddr_18Pin_Alt() per column (~23 cycles), this iterates
 * columns in port-natural order so the innermost loop needs only ONE port write
 * per CAS cycle (~7-8 cycles).
 *
 * Loop nesting (1024 columns = 2 × 8 × 64):
 *   Outer:  PORTC (A0)     → 2 values  + DIN embedded
 *   Middle: PORTB (A7-A9)  → 8 values
 *   Inner:  PORTD (A1-A6)  → 64 values → 1 port write per CAS
 *
...
static void fastPatternTest_18Pin_Alt(uint8_t patNr) {
  ...
  // --- DIN value per A0 state ---
  // DIN is on PC0 (0x01), pattern bit 3 is the relevant data bit for 1Mx1
  uint8_t din_a0_0 = (pat & 0x08) ? 0x01 : 0x00;          // DIN when A0=0
  uint8_t din_a0_1;
  if (patNr <= 1) {
    din_a0_1 = din_a0_0;                                    // Constant pattern
  } else {
    din_a0_1 = (rotate_left(pat) & 0x08) ? 0x01 : 0x00;   // Alternating
  }

  // --- Expected DOUT per A0 state (PC3 = 0x08) ---
  uint8_t exp_dout[2];
  exp_dout[0] = din_a0_0 ? 0x08 : 0x00;  // Expected PINC & 0x08 when A0=0
  exp_dout[1] = din_a0_1 ? 0x08 : 0x00;  // Expected PINC & 0x08 when A0=1

  uint8_t pd_lut[64];
  buildPdLut_18Alt(pd_lut);

  // --- Build PORTC Write values: CAS=HIGH(PC2=1), WE=LOW(PC1=0), A0(PC4), DIN(PC0) ---
  uint8_t pc_upper = PORTC & 0xE0;  // Preserve PC5-PC7
  uint8_t pc_w[2];
  pc_w[0] = pc_upper | 0x04 | din_a0_0;         // A0=0, CAS=HIGH, WE=LOW, DIN for A0=0
  pc_w[1] = pc_upper | 0x04 | 0x10 | din_a0_1;  // A0=1, CAS=HIGH, WE=LOW, DIN for A0=1

  // --- Build PORTC Read values: CAS=HIGH(PC2=1), WE=HIGH(PC1=1), A0(PC4) ---
  uint8_t pc_r[2];
  pc_r[0] = pc_upper | 0x04 | 0x02;         // A0=0, CAS=HIGH, WE=HIGH
  pc_r[1] = pc_upper | 0x04 | 0x02 | 0x10;  // A0=1, CAS=HIGH, WE=HIGH
  ...
  for (uint16_t row = 0; row < 1024; row++) {
    ...
    uint8_t pb_base = PORTB & 0xEA;  // Preserve PB1(unused), PB3(RAS), PB5(LED)

    for (uint8_t pc_i = 0; pc_i < 2; pc_i++) {
      PORTC = pc_w[pc_i];  // Sets A0, DIN, CAS=HIGH, WE=LOW

      for (uint8_t pb_i = 0; pb_i < 8; pb_i++) {
        PORTB = pb_base | pgm_read_byte(&pb_addr_18Alt[pb_i]);
  ...
  for (uint16_t row = 0; row < 1024; row++) {
    ...
    for (uint8_t pc_i = 0; pc_i < 2; pc_i++) {
      PORTC = pc_r[pc_i];  // Sets A0, CAS=HIGH, WE=HIGH
      uint8_t exp = exp_dout[pc_i];

      for (uint8_t pb_i = 0; pb_i < 8; pb_i++) {
        uint8_t pb_val = pgm_read_byte(&pb_addr_18Alt[pb_i]);
        PORTB = pb_base | pb_val;
...
/**
 * Row Write for 18Pin_Alt with port-ordered iteration (random patterns 4-5 only)
 * Patterns 0-3 are handled by fastPatternTest_18Pin_Alt().
 * @param row Row address to write/test
 * @param patNr Pattern number (4-5)
 */
void writeRow_18Pin_Alt(uint16_t row, uint8_t patNr) {
  // Patterns 0-3 are handled by fastPatternTest_18Pin_Alt() — this is only called for 4-5
  ...
  uint8_t pc_w[2];
  pc_w[0] = pc_upper | 0x04;         // A0=0, CAS=HIGH, WE=LOW
  pc_w[1] = pc_upper | 0x04 | 0x10;  // A0=1, CAS=HIGH, WE=LOW

  uint16_t row_mix = row + (row >> 4);  // Precompute row part of mix8
  ...
  cli();
  uint8_t pb_base_w = PORTB & 0xEA;
  for (uint8_t pc_i = 0; pc_i < 2; pc_i++) {
    uint8_t pc_val = pc_w[pc_i];
    uint16_t a0_val = pc_i;

    for (uint8_t pb_i = 0; pb_i < 8; pb_i++) {
      PORTB = pb_base_w | pgm_read_byte(&pb_addr_18Alt[pb_i]);
      uint16_t base_col = a0_val | pgm_read_word(&pb_col_18Alt[pb_i]);

      for (uint8_t pd_i = 0; pd_i < 64; pd_i++) {
        uint16_t col = base_col | ((uint16_t)pd_i << 1);
        uint16_t v = col ^ row_mix;
        uint8_t data = randomTable[(uint8_t)(v ^ (v >> 8))];
        // Set DIN (PC0) in PORTC value
        if (data & 0x08) {
          PORTC = pc_val | 0x01;
        } else {
          PORTC = pc_val & ~0x01;
...
/**
 * Row Check for 18Pin_Alt with port-ordered iteration (random patterns 4-5 only)
 * Patterns 0-3 are handled by fastPatternTest_18Pin_Alt().
 * @param row Row address being tested
 * @param patNr Pattern number (4-5)
 * @param errorNr Error code to report if check fails
 */
void checkRow_18Pin_Alt(uint16_t row, uint8_t patNr, uint8_t errorNr) {
  // Patterns 0-3 read-back is handled by fastPatternTest_18Pin_Alt() — this is only called for 4-5
  ...
  for (uint8_t pc_i = 0; pc_i < 2; pc_i++) {
    ...
    for (uint8_t pb_i = 0; pb_i < 8; pb_i++) {
      ...
      for (uint8_t pd_i = 0; pd_i < 64; pd_i++) {
        uint16_t col = base_col | ((uint16_t)pd_i << 1);
        uint16_t v = col ^ row_mix;
        uint8_t expected_bit = randomTable[(uint8_t)(v ^ (v >> 8))];
