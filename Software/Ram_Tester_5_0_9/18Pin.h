...
// - Checkerboard coupling, pseudo-random, and retention testing
//
//=======================================================================================
...
//
// Address Lines (multiplexed row/column) — matches the setAddr18Pin() asm below and
// the checkAddressShorts mask 0xE7 in checkAddressing_18Pin (5.0.9: block corrected;
// it used to claim A4=PD5/A5=PD4/A6=PD3/A7=PD2, but PD3 is the 18-pin DIP input and
// PD4 is NC — do NOT derive masks/labels from the old values):
//   A0 = PB2   A1 = PB4   A2 = PD7   A3 = PD6
//   A4 = PD2   A5 = PD1   A6 = PD0   A7 = PD5
//
// Control Lines:
...
//
// Address Lines (multiplexed row/column) — matches setAddr18Alt_inl() in 18Pin.cpp:
//   A0       = PC4
//   A1-A3    = PD0-PD2
//   A4-A6    = PD5-PD7   (PD3/PD4 are not address lines here)
//   A7       = PB4
//   A8       = PB2
//   A9       = PB0
//
...
// 2114 SRAM driver for RAM Tester (ATmega328P)
//
// The 2114 is inserted UPSIDE-DOWN (180° rotated in the socket): its Vcc/GND are
// swapped relative to the tester's 18-pin power rails, so flipping the chip realigns
// power at the cost of rotating every signal pin by 180°. Resulting port mapping
// (implemented in 18Pin.cpp drive_addr_2114 / drive_data_2114 / sram2114_read):
//   A0=PB2  A1=PB0  A2=PC2  A3=PB4  A4=PD7
//   A5=PD6  A6=PD5  A7=PD2  A8=PD1  A9=PD0
//   IO1=PC4 IO2=PB1 IO3=PB3 IO4=PC1
//   _CS=PC3 _WE=PC0
//
// The internal D-bit to IO-line order is arbitrary (write and read use one
// consistent permutation). Verify against schematic if a nibble reads scrambled.

// The ADDR_MASK_* / IO_MASK_* 2114 port masks are defined file-locally in 18Pin.cpp
// (next to the 2114 driver). The stale duplicates that used to live here were removed
// in 5.0.3 — they shadowed the .cpp values (and held an outdated pin mapping), which
// triggered -Wall macro-redefinition warnings.
...
// Optimized storage: high bits in an 8-byte LUT; low bits (A1-A6) computed
// arithmetically in setAddr_18Pin_Alt (vs 1024 bytes for a combined table).
 ...
 * 4. Pattern testing (checkerboard coupling 0-1 + pseudo-random 4-5)
...
/**
 * Write random test pattern to entire row using page mode (4416/4464)
 *
 * Performs optimized page mode write of random data to all columns in a row.
 * Supports 4-bit data bus with column-by-column testing. Only used for the
 * pseudo-random patterns (the checkerboard passes run through checkerRow_18Pin).
 *
 * @param row Row address to write (0-63 for 4416, 0-255 for 4464)
 * @param patNr Pattern number (4 = random, 5 = random inverted)
 * @param width Number of columns to write (64 for 4416, 256 for 4464)
 */
void writeRow_18Pin(uint16_t row, uint8_t patNr);  // random + retention (4-5), 5.0.3
...
/**
 * Verify random test pattern in row using page mode read (4416/4464)
 *
 * Reads back and verifies previously written random data using 4-bit data bus.
 *
 ...
 */
void checkRow_18Pin(uint16_t row, uint8_t patNr);  // retention read-back (4-5), 5.0.3
...
/**
 * Write random test pattern to entire row using page mode (411000)
 *
 * Performs optimized page mode write similar to 16-pin implementation
 * but using alternative interface pinout and lookup tables. Only used for the
 * pseudo-random patterns (the checkerboard runs through checkerboardTest_18Pin_Alt).
 *
 * @param row Row address to write (0-1023 for 411000)
 * @param patNr Pattern number (4 = random, 5 = random inverted)
 */
void writeRow_18Pin_Alt(uint16_t row, uint8_t patNr);  // random + retention (4-5), 5.0.3

/**
 * Verify random test pattern in row using page mode read (411000)
 *
 * Reads back and verifies previously written random data using 1-bit data bus.
 *
 ...
 */
void checkRow_18Pin_Alt(uint16_t row, uint8_t patNr);  // retention read-back (4-5), 5.0.3
...
// Presence Check for 2114 SRAM
bool ram_present_2114(void);

// Address-line check (physical short + walking-bit functional test) for 2114 SRAM
void checkAddressing_2114(void);

// _CS / _WE functional check for 2114 SRAM
void check_CSWE_2114(void);

// Full pattern test (March C-) for 2114 SRAM; never returns on failure
