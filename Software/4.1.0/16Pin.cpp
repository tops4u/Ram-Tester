...
void __attribute__((hot)) writeRow_16Pin(uint16_t row, uint16_t cols, uint8_t patNr) {
  ...
  if (row == last_row) {
    // Last row: verify all still-pending delayed rows (oldest first).
    //
    // Normal path timing for checking row R (written delayRows rows ago):
    //   [Write R] ... delayRows × (writeTime + delays[5] + checkTime) ... [checkRow R]
    //   The writeTime of each subsequent row creates implicit delay between write and read.
    //
    // At last_row the oldest pending row (last_row - delayRows) has already accumulated
    // its full writeTime delay from the Write of last_row itself.  So the first check
    // needs only delays[5].  But for each subsequent check (x < delayRows) there is no
    // further row write — we must simulate the missing writeTime explicitly.
    //
    // x = delayRows:     delays[5]               → checkRow(last_row - delayRows)
    // x = delayRows-1:   writeTime + delays[5]   → checkRow(last_row - delayRows + 1)
    // ...
    // x = 0:             writeTime + delays[5]   → checkRow(last_row)
    for (int8_t x = ramTypes[type].delayRows; x >= 0; x--) {
      if (x < ramTypes[type].delayRows)
        delayMicroseconds(ramTypes[type].writeTime * 20);  // Simulate missing row write
    ...
    // writeTime has already elapsed implicitly through writing of subsequent rows.
    delayMicroseconds(ramTypes[type].delays[5] * 20);
...
static void refreshTimeTest_16Pin() {
  ...
  // ===== PHASE 2: PERFORM 10 FULL CAS-BEFORE-RAS REFRESH SEQUENCES (40ms) =====
  // Tests the internal CBR refresh counter:
  // - 10 full cycles ensures counter wraps around multiple times
  // - 10× the spec refresh time: cells will lose data if refresh fails
  // - 256 CBR cycles per sequence covers all 512 rows
