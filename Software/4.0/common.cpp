 ...
 * Uses U8G2 library with page buffer mode (suffix _1) to save RAM
 */
#ifdef OLED
...
struct RAM_Definition ramTypes[] = {
  ...
  { ramType_4164, 4, 2, 256, 256, RAM_FLAG_SMALL_TYPE, { 57, 56, 9, 9, 9, 9 }, 44 },
  { ramType_41256, 4, 1, 512, 512, 0, { 114, 20, 20, 20, 20, 20 }, 85 },
  { ramType_41257, 4, 2, 512, 512, RAM_FLAG_NIBBLE_MODE, { 114, 20, 20, 20, 20, 20 }, 85 },
  ...
  { ramType_411000, 8, 1, 1024, 1024, 0, { 209, 25, 25, 25, 25, 25 }, 166 },
  { ramType_4116, 2, 2, 128, 128, 0, { 30, 30, 6, 6, 6, 6 }, 24 },
  { ramType_4816, 2, 2, 128, 128, 0, { 28, 28, 4, 4, 4, 4 }, 24 },
...
void initRAM(uint8_t RASPin, uint8_t CASPin) {
  delayMicroseconds(250);      // Power stabilization delay
...
void error(uint8_t code, uint8_t error, int16_t row, int16_t col) {
  ...
  // Enter infinite LED blink pattern based on error type
  switch (error) {
    ...
    case 2:  // RAM faulty - 2 red, then orange count based on pattern
    case 3:  // Retention error - 2 red, 7 orange (merged handling)
      while (true) {
        ...
        if (error == 3) {
          orange_count = 7;  // Retention error = 7 orange blinks
        } else if (code <= 4) {
          orange_count = code + 1;  // Patterns 0-4 = 1-5 orange blinks
        } else {
          orange_count = 6;  // Random test failure = 6 orange blinks
        }
    ...
    case 5:  // Refresh timeout - 2 red, 8 orange
      while (true) {
        blinkLED_color(LED_RED, 2, BLINK_ON_MS, BLINK_OFF_MS);
        delay(INTER_BLINK_MS);
        blinkLED_color(LED_ORANGE, 8, BLINK_ON_MS, BLINK_OFF_MS);
        delay(ERROR_PAUSE_MS);
      }
      break;
  }
