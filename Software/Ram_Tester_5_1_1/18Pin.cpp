...
const uint8_t CPU_18PORTB[] = { 15, 4, 14, 3, 13, EOL, EOL, EOL };
// PB4 = socket pin 13 (owner-confirmed): the same physical pin as 20-pin 15 and
// 16-pin 11. It carries A1 on the 4416/4464 and A9 on the 411000 (see the pinout
// blocks below), and both address-short masks already drive it (0x14 / 0x15) — but
// the entry read EOL, so the GND check and the VCC probe skipped socket pin 13
// entirely. A short to ground there only surfaced later as an address-line fault.
...
bool ram_present_18Pin(void) {
  ...
  setAddr18Pin(0x00);
  CAS_LOW18;
  // tCAC margin: two NOPs sample at 31-94 ns, under the 75-100 ns tCAC of a -15/-20
  // grade 4416/4464 — a good part then mis-senses. Four NOPs give 156-219 ns.
  NOP;
  NOP;
  NOP;
  NOP;
...
bool ram_present_18Pin_alt(void) {
  ...
  NOP;
  NOP;
  NOP;  // tCAC margin, see read_addr_alt_18Pin
...
static uint8_t __attribute__((noinline)) col_read(uint8_t col) {
  ...
  NOP;
  NOP;
  NOP;  // tCAC: getData18Pin is "in PINC; in PINB", so the two halves land at
        // 156-219 ns and 219-281 ns. With 2 NOPs the PINC half was at 31-94 ns,
        // below the 75-100 ns tCAC of a -15/-20 grade.
 ...
 * part: the march needs a clean 0/1 per address, and the other three data lines are
 * covered far better by the pattern passes than a single walking bit ever did.
 * A rolling RAS-only refresh per access keeps the array alive -- the 4416 predates CBR,
 * and 256 rows at ~10 us per read+write step put the worst write-to-read gap (about two
 * march elements, ~5 ms) past the 4 ms tREF without it.
 */
static uint16_t march18Ref = 0;

static uint8_t march18Pin(uint16_t r, uint16_t c, uint8_t op) {
  refreshRow_18Pin((uint8_t)march18Ref++);
  if (type == T_4416) c <<= 1;  // 4416: columns sit on A1..A6
  if (op == 2) {
    configDataIn_18Pin();
    OE_LOW18;
    uint8_t v = read18Pin(r, (uint8_t)c) & 1;
    OE_HIGH18;
    return v;
  }
  configDataOut_18Pin();
  WE_LOW18;
  write18Pin(r, (uint8_t)c, op);
  WE_HIGH18;
  return 0;
}
...
void checkAddressing_18Pin() {
  ...
  // 5.1.1: MATS+ over each axis replaces the walking-1 loops. Those compared address 0
  // against the 2^b peers only; unique peer data (0xA^b) was added in 5.0.10 so that two
  // peers aliasing onto EACH OTHER would at least not read back a common value, but a
  // pair with no bit-0 partner still passed, and so did the pattern tests, which verify
  // a row before the aliasing row is written. The march covers every pair and still
  // names the line for a dead/stuck A<b> (it trips at exactly a = 2^b, see marchAxes).
  // Both 18-pin types have 256 rows; the 4416 has 64 columns on A1..A6, the 4464 256.
  marchAxes(256, (type == T_4416) ? 64 : 256, &march18Pin);
}
...
static void __attribute__((hot)) pass_18Pin(uint8_t patNr, bool runCBR) {
  ...
  for (uint16_t row = 0; row < 256; row++) {
    ...
    for (uint16_t col = 0; col < 256; col++) {
      ...
      NOP;
      NOP;
      // Read while CAS is still LOW. getData18Pin is "in PINC; in PINB"; the PINB half
      // used to land at 156-219 ns with CAS rising at 187 ns — partly past the edge,
      // where the outputs may already be Hi-Z and the PB0/PB3 pull-ups read 1.
      uint8_t got = getData18Pin();
      CAS_HIGH18;
      if (got != d) {
        sei();
        OE_HIGH18;
...
static void pass_4416_LAG(uint8_t patNr) {
  ...
  for (uint16_t i = 0; i < 256 + LAG; i++) {
    ...
    if (i < 256) {
      ...
      configDataOut_18Pin();
      cli();  // BEFORE RAS: this was the one 13.8 us window measured per 4416 run
    ...
    if (i >= LAG) {
      ...
      configDataIn_18Pin();
      OE_LOW18;
      cli();  // BEFORE RAS, see the write branch
...
void checkRow_18Pin(uint16_t row, uint8_t patNr) {
  ...
  // mix8's row half, computed BEFORE the row opens. Left inside, GCC puts it in the loop
  // preheader — i.e. in the first RAS-low window, which measured 10.62 us against the
  // 4416's 10 us tRAS max while the steady-state windows sat at 8.69 us (LA, 5.1.1).
  ...
  for (uint16_t col = 0; col < width; col++) {
    if (col_read((uint8_t)(col << init_shift)) != randomTable[mix8r(col, row_mix)]) {
      sei();
      OE_HIGH18;
      RAS_HIGH18;
      error(patNr, 3);
...
void writeRow_18Pin(uint16_t row, uint8_t patNr) {
  ...
  RAS_LOW18;
  for (uint16_t col = 0; col < width; col++) {
    col_write((uint8_t)(col << init_shift), randomTable[mix8r(col, row_mix)]);
    if (is4416) {
      rasHandling_18Pin(row);  // 4416: unchanged full recycle (calibration preserved)
    } else {
...
// Defined below, used here by sense411000_18Pin_Alt.
...
void sense411000_18Pin_Alt() {
  ...
  // 5.0.11: the dedicated write_read/read_alt pair was just the row==col==addr special
  // case of write_alt_18Pin/read_addr_alt_18Pin and carried its own (shorter) tCAC delay.
  // Using the general helpers deletes ~80 bytes, gives these three accesses the same
  // sampling margin as the address test, and puts them under the same cli/sei shield.
  write_alt_18Pin(0, 0, 0);
  uint8_t test1 = read_addr_alt_18Pin(0, 0);

  write_alt_18Pin(1, 1, 1);
  uint8_t test2 = read_addr_alt_18Pin(1, 1);

  // Cross-check: address 0 must still be 0
  uint8_t test3 = read_addr_alt_18Pin(0, 0);
...
static inline uint8_t read_addr_alt_18Pin(uint16_t row, uint16_t col) {
  ...
  setAddr_18Pin_Alt(col);
  CAS_LOW_18PIN_ALT;
  // Four NOPs: the synchroniser aperture is [T-1.5, T-0.5] cycles, so two put it at
  // 31-94 ns — under the 20-30 ns tCAC of a -70/-10 411000 once adapter trace delay and
  // socket contact resistance are added. Four moves it to 156-219 ns.
  NOP;
  NOP;
  NOP;
  NOP;
...
/**
 * Cell accessor for the decoder-alias march (see marchAxes in common.h). One CBR per
 * access: a 1024-address element runs ~8 ms, right at the part's 8 ms tREF, and the
 * worst write-to-read gap spans two of them.
 */
static uint8_t march18Alt(uint16_t r, uint16_t c, uint8_t op) {
  casBeforeRasRefresh_18Pin_Alt();
  if (op == 2) return read_addr_alt_18Pin(r, c);
  write_alt_18Pin(r, c, op);
  return 0;
}
...
void checkAddressing_18Pin_Alt() {
  ...
  // mC 0x00 -> 0x10: A0 is PC4 in the Alt pinout (setAddr18Alt_inl), so it was the only
  // address line never short-tested.
  checkAddressShorts(0x15, 0x10, 0xE7);
  ...
  // 5.1.1: MATS+ over each axis (411000 = 1Mx1, 1024 rows x 1024 cols), replacing the
  // walking-1 loops. This is the type that needed it most: x1 leaves no room for unique
  // peer data, so the old test could not see peer-onto-peer aliasing AT ALL, and with
  // delayRows = 1 the pattern pipeline checks row N-1 before row N+k is ever written.
  marchAxes(1024, 1024, &march18Alt);
}
