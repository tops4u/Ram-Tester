...
The goal was to make it work lightning fast!b 
...
uint8_t eeprom_counter_with_wear_leveling(void) {
    ...
    uint8_t value;
    uint8_t parity;
    // Find current active address (first 0xFF cell or last used cell)
    for (addr = EEPROM_START_ADDR; addr <= EEPROM_END_ADDR; addr++) {
        if (eeprom_read_byte((uint8_t*)addr) == 0xFF) break;
    }
    // If we found 0xFF, active address is the previous one
    if (addr > EEPROM_START_ADDR && addr <= EEPROM_END_ADDR) {
        addr--;
        value = eeprom_read_byte((uint8_t*)addr);
    }
    // If no 0xFF found, use last address
    else if (addr > EEPROM_END_ADDR) {
        addr = EEPROM_END_ADDR;
        value = eeprom_read_byte((uint8_t*)addr);
    }
    // If first address is 0xFF (fresh EEPROM), start there
    else {
