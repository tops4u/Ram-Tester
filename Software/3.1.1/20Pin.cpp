...
//=======================================================================================
// TIMING DELAY FUNCTIONS - Regular functions, compiler decides optimization
// CALL+RET = ~4 cycles (250ns @ 16MHz), reducing needed NOPs
//=======================================================================================

/**
 * Short delay: ~125ns target
 * CALL+RET = 4 cycles (250ns) - already sufficient!
 * No NOPs needed - function overhead provides the delay
 */
void delay_short(void) {
  // Function call overhead (CALL = 4 cycles, RET = 4 cycles) = 8 cycles total
  // At 16MHz: 8 cycles = 500ns
  // This is MORE than enough for most timing requirements
}

/**
 * Medium delay: ~250-375ns target
 * CALL+RET = 8 cycles base + 1 NOP
 */
void delay_medium(void) {
  NOP;  // Add 1 cycle for slightly longer delay
}

/**
 * Long delay for critical read operations: ~500ns
 * CALL+RET = 8 cycles base + 3 NOPs
 */
...
uint8_t readCell_4116(uint8_t col) {
  ...
  delay_short();
  CAS_LOW20;
  delay_long();  // Longer delay for read access time
...
//=======================================================================================
// PATTERN WRITE FUNCTIONS
//=======================================================================================

/**
 * Write stuck-at pattern with inline verification (patterns 0-1)
 * @param row Row address
 * @param pat Pattern value
 */
void writeStuckAtPattern_4116(uint8_t row, uint8_t pat) {
...
/**
 * Write walking pattern (patterns 2-3)
 * @param pat Initial pattern value
 */
void writeWalkingPattern_4116(uint8_t pat) {
...
/**
 * Write random pattern (pattern 5)
 * @param row Row address
 */
void writeRandomPattern_4116(uint8_t row) {
  uint8_t cols = (uint8_t)ramTypes[type].columns;
  for (uint8_t col = 0; col < cols; col++) {
    writeCell_4116(col, get_test_bit(col, row));
  }
}

/**
 * Handle retention testing delays
 * @param row Current row being tested
 */
void handleRetentionDelay_4116(uint8_t row) {
  uint8_t delayRows = ramTypes[type].delayRows;
  uint8_t delayIdx = (row < delayRows) ? row : delayRows;
  delayMicroseconds(ramTypes[type].delays[delayIdx]);
}
...
void writeRow_4116(uint8_t row, uint8_t patNr) {
  ...
  if (patNr < 2) {
    writeStuckAtPattern_4116(row, pat);
    sei();
    RAS_HIGH20;
    return;
  } else if (patNr < 4) {
    writeWalkingPattern_4116(pat);
  } else {
    writeRandomPattern_4116(row);
  }
  ...
  refreshRow_4116(row);
  uint8_t delayRows = ramTypes[type].delayRows;
  uint8_t maxRows = ramTypes[type].rows;

  if (row == maxRows - 1) {
    for (int8_t x = delayRows; x >= 0; x--) {
      checkRow_4116(row - x, patNr, 3);
      delayMicroseconds(ramTypes[type].writeTime + ramTypes[type].delays[delayRows]);
    }
    return;
  }

  if (row >= delayRows) {
    checkRow_4116(row - delayRows, patNr, 3);
  }

  handleRetentionDelay_4116(row);
}
...
void checkRow_4116(uint8_t row, uint8_t patNr, uint8_t errorNr) {
  ...
  for (uint8_t col = 0; col < cols; col++) {
    ...
    if ((readCell_4116(col) ^ expected) != 0) {
      sei();
