...
void test_20Pin(void) {
  ...
  do {
    // Address test INSIDE the loop (5.1.1) -- see test_16Pin: loop mode is the warm test,
    // so the march runs warm as well. ~50 ms on a 512x512 part, ~96 ms on a 1M one.
...
void writeRow_4116(uint8_t row, uint8_t patNr) {
  ...
  uint8_t cols = (uint8_t)ramTypes[type].columns;
  cli();  // BEFORE RAS: the ISR (~6 us) in the first window pushes it over tRAS max 10 us
  rasHandling_4116(row);
  WE_LOW20;
...
void checkRow_4116(uint8_t row, uint8_t patNr, uint8_t errorNr) {
  uint8_t cols = (uint8_t)ramTypes[type].columns;
  cli();  // BEFORE RAS, see writeRow_4116
  rasHandling_4116(row);
...
/**
 * Cell accessor for the decoder-alias march (see marchAxes in common.h).
 * The 4116/4027 predate CAS-before-RAS, so the array is kept alive by a rolling RAS-only
 * sweep: 128 rows at ~12 us per read+write step put the worst write-to-read gap (about
 * two march elements, ~3 ms) past the 2 ms tREF without it. The mask keeps A6 low on the
 * 4027, where that pin is CS and a "refresh" with it high would deselect the chip.
 */
static uint8_t march4116Ref = 0;

static uint8_t march4116(uint16_t r, uint16_t c, uint8_t op) {
  refreshRow_4116(march4116Ref++ & ((type == T_4027) ? 0x3F : 0x7F));
  if (op == 2) {
    rasHandling_4116((uint8_t)r);
    uint8_t v = readCell_4116((uint8_t)c);
    RAS_HIGH20;
    NOP;
    NOP;  // tRP (extra for 4027)
    return v ? 1 : 0;
  ...
  WE_LOW20;
  writeAddressTest_4116((uint8_t)r, (uint8_t)c, op);  // carries its own RAS_HIGH + tRP
  WE_HIGH20;
  return 0;
}
...
void checkAddressing_4116(void) {
  ...
  const uint16_t n = (type == T_4027) ? 64 : 128;  // both geometries are square
  ...
  // 5.1.1: MATS+ over each axis replaces the walking-1 loops, which compared address 0
  // against the 2^b peers only and — being x1, with no room for unique peer data — could
  // not see two peers aliasing onto each other at all. A dead/stuck A<b> still reports as
  // "Addressline A<b>": the up-element trips at exactly a = 2^b (see marchAxes).
  marchAxes(n, n, &march4116);
}
...
// PB4 (A8, socket pin 15) is now GND-checked. It used to be EOL, so a chip with A8
// shorted to GND passed the check and configureIO_20Pin then drove an OUTPUT HIGH
// into the short; the fault surfaced later as a misleading "Addressline A8".
// LBL_20B[4] already carried the A8 label for a slot that could never be reached.
const uint8_t CPU_20PORTB[] = { 17, 4, 16, 3, 15, EOL, EOL, EOL };
...
static inline bool test_ram_presence_20Pin(void) {
  ...
  OE_HIGH20;
  CAS_LOW20;
  // tCAC margin: two NOPs sample at 31-94 ns. This runs before anything else, so a
  // marginal socket/adapter here yields "Defect or no RAM!" on a good chip.
  NOP;
  NOP;
  NOP;
  NOP;
  ...
  NOP;
  NOP;
  NOP;  // tCAC margin, see the pull-up read above
...
bool ram_present_20Pin(void) {
  PORTC &= 0xf0;  // data nibble = 0 ...
  DDRC |= 0x0f;   // ... and DRIVEN: test_ram_presence_20Pin writes 0 to (0,0) first.
                  // Was "DDRC &= 0xf0" (inputs, no pull-up), so that write strobed CAS
                  // over an undriven bus and only worked on residual charge.
  RAS_HIGH20;
  CAS_HIGH20;
  OE_HIGH20;
  WE_HIGH20;
...
static inline void write20Pin(uint16_t row, uint16_t col, uint8_t data) {
  rasHandling_20Pin(row);  // RAS falls with WE HIGH: WE low at RAS↓ arms the
                           // write-per-bit mask on WPB parts (e.g. µPD424401) and
                           // masks the write; don't-care for standard FPM/SC.
  WE_LOW20;                // early write: still well before CAS↓ (tWCS ok)
 ...
 */
static uint8_t read20Pin(uint16_t row, uint16_t col) {
  rasHandling_20Pin(row);
  msbHandling_20Pin(col >> 8);
  PORTD = (uint8_t)(col & 0xFF);
  NOP;
  CAS_LOW20;
  // 5.0.11: sample while CAS is still LOW. Reading after CAS_HIGH20 relied on the bus
  // HOLDING its level past the rising edge — true on a ZIP socket, whose capacitance
  // keeps it ~1.7 us, but never guaranteed: tOFF(min) is 0 ns. On an SOJ adapter the
  // longer, lighter lines let the level go, which surfaced as "Addressline A0"
  // (the row loop checks the base cell first, so any misread is blamed on b=0).
  // THREE NOPs, not two: the synchroniser aperture is [T-1.5, T-0.5] cycles, so with two
  // it starts at 31 ns -- only ~14 ns of margin over the tCAC of a -60 part once the chip
  // itself has used 17 ns, and that has to cover adapter trace delay AND socket contact
  // resistance (which adds R*C to the DOUT edge: ~4 ns clean, ~10 ns on a tarnished pin).
  // A customer on an SOJ adapter saw the test pass right after cleaning the ZIF and fail
  // again later -- exactly what sitting on that edge looks like. Three NOPs move the
  // aperture to 94-156 ns, ~5x the chip's tCAC, still far inside the CAS-low window.
  NOP;
  NOP;
  NOP;
 ...
 */
/**
 * Cell accessor for the decoder-alias march (see marchAxes in common.h). Only DQ1 takes
 * part -- the march wants a clean 0/1 per address, and the other three data lines are
 * covered far better by the pattern passes than a walking bit ever was.
 * One CBR per access: a 512-address element runs ~4 ms and a 1024-address one ~8 ms,
 * and the worst write-to-read gap spans two of them -- i.e. right at the 8 ms (514256)
 * and 16 ms (514400) tREF without help.
 */
static uint8_t march20Pin(uint16_t r, uint16_t c, uint8_t op) {
  casBeforeRasRefresh_20Pin();
  if (op == 2) {
    PORTC &= 0xF0;  // release the data nibble before turning it around
    DDRC &= 0xF0;   // PC0-PC3 input; PC4 (A9 on the 1M types) stays an output
    OE_LOW20;
    uint8_t v = read20Pin(r, c) & 1;
    OE_HIGH20;
    return v;
  }
  DDRC |= 0x0F;  // data nibble output
  write20Pin(r, c, op);
  return 0;
}
...
void checkAddressing_20Pin() {
  ...
  // 5.1.1: MATS+ over each axis replaces the walking-1 loops. Those wrote unique peer
  // data (b+1) so that two peers aliasing onto each other could not read back a common
  // value, but they still only ever compared address 0 against the 2^b peers -- a pair
  // with no power-of-two partner passed, and so did the pattern tests, which verify a
  // row before the aliasing row is written. A clean SWAP of two address lines stays
  // invisible to both (write and read go through the same decoder); everything else is
  // now covered. A dead/stuck A<b> still reports as "Addressline A<b>" (the up-element
  // trips at exactly a = 2^b, see marchAxes).
  marchAxes(ramTypes[type].rows, ramTypes[type].columns, &march20Pin);
}
...
void senseRAM_20Pin() {
  ...
  CAS_LOW20;
  NOP;
  NOP;  // tCAC margin: 31-94 ns was enough on a ZIP socket but not with an adapter's
        // extra trace delay -- a misread here mistypes a 514400 as a 514256.
  CAS_HIGH20;
...
void senseSCRAM_20Pin() {
  ...
  // 5.0.8: position = i*5 ({0,5,10,15}) computed inline -> drops the 4-byte stack array.
...
void writeRow_20Pin(uint16_t row, uint8_t patNr, boolean is_static) {
  ...
  if (is_static) {
    ...
    // Fast-page write (514256/514400), 2x unrolled. WE held low within the bursts
    // but raised across each RAS recycle (write-per-bit mask guard, see below).
    WE_LOW20;
    for (uint8_t msb = 0; msb < msbCol; msb++) {
      msbHandling_20Pin(msb);
