...
// PB4 = A4 (socket pin 11), shared with the green LED / OLED SDA — included since
// 5.0.9: setup() releases PB4 to input+pull-up right before checkGNDShort (the OLED
// driver parks PB4/PB5 as OUTPUT after each transfer, which would mask the short).
// If A4 IS shorted to GND the OLED (shared SDA) is dead anyway — the LED blink code
// 3 red + 11 orange still reports it.
const uint8_t CPU_16PORTB[] = { 13, 4, 12, 3, 11, EOL, EOL, EOL };  // PB0-PB4 used
...
static uint8_t march16Pin(uint16_t r, uint16_t c, uint8_t op);  // march accessor (below)
...
// Deferred march verdict for T_4164 with 32K detection on (see checkAddressing_16Pin and
// the quadrant evaluation in test_16Pin). marchScan encoding; 0 = the part passed 256x256.
static uint16_t march4164;
...
void test_16Pin() {
  ...
  if (CFG_32K_ACTIVE && type == T_4164)
    ...
    writeRAMType(RAM_NAME());

  // Restore I/O configuration after chip sensing
  DDRB = 0b00111111;
  PORTB = 0b00101010;

  // Arm the T_4164 4ms->2ms retention retry ONCE per RESET (5.0.9; was re-armed
  // every loop iteration inside run_16Pin_tests). In loop mode only iteration 1
  // may downgrade to T_4164_2MS; a chip that passes 4 ms cold but fails it warm
  // in a later stress iteration now ABORTS (32K off: immediate retention error;
  // 32K on: the loopQuadrantSnapshot guard below) instead of silently being
  // retested at 2 ms under the stale iteration-1 "4ms" OLED label.
  ...
  // pass. run_16Pin_tests() resets quadrantErrors internally; retryActive is
  // armed once above and consumed/cleared by the pat45 retry logic.
  do {
    // Address test INSIDE the loop (5.1.1): loop mode is the warm/soak test, so the march
    // has to run warm too -- a decoder that only aliases after the part has been busy for
    // twenty minutes, or a cell in the marched line that stops holding, is exactly what
    // this mode exists for. It also brings 16-pin in line with 18-pin, which has always
    // run its address test per iteration. Costs ~45 ms on a ~10 s iteration.
    ...
    if (CFG_32K_ACTIVE && (type == T_4164 || type == T_4164_2MS)) {
      ...
      if (loopQuadrantSnapshot == 0xFF) {
        ...
        if (quadrantErrors) {
          ...
          // ===== MARCH THE HALF THIS PART CAN ACTUALLY DO (5.1.1) =====
          // The full-array scan in checkAddressing_16Pin was bound to fail here -- half
          // the array is dead by design -- and its verdict was just discarded. But the
          // part is sold on the promise that its GOOD half is a working 16K, and nothing
          // has checked THAT half for decoder aliases either: the checkerboard and the
          // random passes are LAG-2 pipelines and structurally cannot see one (see
          // checkAddressing_16Pin). So march the good quadrant pair.
          // Only the four DEFINITIVE splits (two quadrant bits) — with a single ambiguous
          // quadrant the good region is not a half and there is nothing safe to hold.
          //   0x03: rows 0..127 dead   -> rows 128..255 x cols 0..255, origin (128, 0)
          //   0x0C: rows 128..255 dead -> rows 0..127   x cols 0..255, origin (0, 0)
          //   0x05: cols 0..127 dead   -> rows 0..255   x cols 128..255, origin (0, 128)
          //   0x0A: cols 128..255 dead -> rows 0..255   x cols 0..127, origin (0, 0)
          // The origin doubles as the held coordinate of the other axis (see marchOrgR/C),
          // so the probe never touches a dead row or column. A failure here means the part
          // is not usable as a 16K device either, and IS reported: the relative address is
          // the right one to name, because the good half is this part's whole array.
          // Restore the address-test port state — the pattern passes leave RAS/CAS/WE and
          // the PC1 (Din) direction wherever their last access happened to end.
          if (quadrantErrors & (quadrantErrors - 1)) {
            uint8_t rowSplit = (quadrantErrors == 0x03 || quadrantErrors == 0x0C);
            DDRB = 0b00111111;
            ...
            DDRD = 0b11000011;
            PORTD = 0x00;
            marchOrgR = (quadrantErrors == 0x03) ? 128 : 0;
            marchOrgC = (quadrantErrors == 0x05) ? 128 : 0;
            marchAxes(rowSplit ? 128 : 256, rowSplit ? 256 : 128, &march16Pin);
          }
        } else if (march4164) {
          // ===== DEFERRED MARCH VERDICT — plain 4164 =====
          // No quadrant errors, so this is not a half-good part: the 256x256 march that
          // checkAddressing_16Pin ran really did find a fault, and now it can be named.
          if (march4164 & 0x8000) error(0, 2);         // dead cell, not an address fault
          else error((uint8_t)(march4164 - 1), 1);     // "Addressline A<b>" / "Decoder Alias"
        }
        ...
        // The march reruns every iteration now, so read its verdict here too -- but only
        // for a part the baseline established as a plain 4164. On a half-good one it
        // fails by design, exactly as in iteration 1, and is discarded the same way.
        if (loopQuadrantSnapshot == 0 && march4164) {
          if (march4164 & 0x8000) error(0, 2);
          else error((uint8_t)(march4164 - 1), 1);
        }
      }
    }
...
static void run_16Pin_tests() {
  ...
  // T_4164 4ms->2ms retry: retryActive is armed ONCE per reset in test_16Pin
  // (5.0.9) — no longer here. Re-arming every loop iteration let a warm-degrading
  // chip downgrade silently mid-stress-test and keep "passing" at 2 ms.
...
static inline void write16Pin(uint16_t row, uint16_t col, uint8_t data) {
  ...
  RAS_LOW16;
  WE_LOW16;
  // DIN back to OUTPUT: read16Pin releases it (shared Din/Dout parts) and SET_DIN_16
  // only drives PORTC, so every write after a read used to reach the chip through the
  // internal pull-up alone -- and a '0' through nothing at all, latching whatever the
  // ~5 pF DIN node still held. Harmless in the old walking-1 order (writes came first),
  // but the march reads and writes the same cell back to back. WE is already LOW, so the
  // chip's output is disabled and driving the shared pin here cannot contend.
  DDRC |= 0x02;
...
static inline uint8_t read16Pin(uint16_t row, uint16_t col) {
  ...
  CAS_LOW16;
  NOP;
  CAS_HIGH16;  // tCAC: sample only AFTER CAS_HIGH16 (94-156 ns), like checkRow_16Pin.
  uint8_t result = (PINC & 0x04) >> 2;  // was 2 NOPs then read = 31-94 ns, under tCAC
  RAS_HIGH16;
  sei();
...
/**
 * Cell accessor for the decoder-alias march (see marchDecoder in common.h).
 * A CBR refresh per write keeps the array alive: the 512-row types sweep ~2.6 ms per
 * march element, and the worst-case gap between a row's write in one element and its
 * read in the next is about two element lengths -- over the 4 ms tREF without help.
 */
static uint8_t march16Pin(uint16_t r, uint16_t c, uint8_t op) {
  if (op == 2) return read16Pin(r, c);
  write16Pin(r, c, op);
  casBeforeRasRefresh_16Pin();
  return 0;
}
...
static void detect41257_16Pin() {
  ...
  // Nibble 0 → read col 0
  // All four nibble reads sample AFTER CAS_HIGH16 (aperture 94-156 ns), matching
  // fastPatternNibble_16Pin. They used to read 2 NOPs after CAS↓ = 31-94 ns, under the
  // tCAC of a slow grade -> v0 reads 0 -> a good 41257 is demoted to 41256 and then
  // driven in page mode, which this part does not support.
  CAS_LOW16;   // CAS↓: latch col 0
  NOP;
  CAS_HIGH16;  // CAS↑: counter → col 1
...
void checkAddressing_16Pin(void) {
  ...
  // ========== DECODER-ALIAS MARCH (replaces the walking-1 address test) ==========
  // 5.1.1: the old walking-1 loops only compared address 0 against the 2^b peers, so an
  // alias between two arbitrary rows (or columns) walked straight through them -- and
  // through the pattern tests too, because those verify a row before the aliasing row
  // has been written. MATS+ over each axis closes that completely and still names the
  // line for the fault the walking-1 test was there for: a dead or stuck A<b> makes a
  // and a^2^b alias, so the up-element trips at exactly a = 2^b (see marchAxes).
  // 4164 with 32K detection on: scan the FULL array, but do not act on the result yet.
  // A plain 4164 must pass all 256x256 by definition; a half-good 4532/3732 must NOT,
  // because half of its array is dead or aliased BY DESIGN. So the march is itself the
  // sharpest 32K indicator there is -- what it cannot do is tell which of the two it is
  // looking at, and reporting here would throw out a good 4532 before the quadrant
  // evaluation ever gets to classify it (a TMS4532-4 has rows 0..127 dead and would fail
  // at the very first address). The verdict is therefore held until that evaluation:
  //   quadrantErrors != 0 -> half-good part, the march failure is explained, discard it
  //   quadrantErrors == 0 -> a plain 4164, so the failure was real -> report it there
  // This is the same deferral the old walking-1 A7 step used, just with full coverage.
  // It also matters that NOTHING ELSE on this type can see a decoder alias: the
  // checkerboard (checkerQuadrant_16Pin, LAG 2) and the random passes (delayRows 2) read
  // row j at step j+2, i.e. before an aliasing row j+k (k > 2) has been written, so
  // neither read ever sees wrong data -- which is equally why a genuine alias leaves
  // quadrantErrors at 0 and lands in the branch that reports it.
  if (CFG_32K_ACTIVE && type == T_4164) {
    march4164 = marchScan(256, 256, &march16Pin);
    // Report NOW unless the failure is one a half-good part produces BY DESIGN. Exactly
    // three results are ambiguous: the row A7 boundary (code 7 -> 8 here), the column A7
    // boundary (code 23 -> 24) and a dead cell (0x8000) — a TMS4532 is split at A7
    // row-wise, an MSM3732 column-wise, and a dead half surfaces as a dead cell. Nothing
    // else can be a clean split, so it is a real fault and has to be named here.
    // 5.1.1 fix: deferring EVERY failure to the quadrant evaluation was wrong — the
    // pattern passes run first and abort on their own, so a stuck address line on any
    // pin but A7 was reported as the far vaguer "Error Checkerboard"/"Error RandomData"
    // while the march had already identified the line.
    if (march4164 && march4164 != 0x8000 && march4164 != 8 && march4164 != 24)
      error((uint8_t)(march4164 - 1), 1);
    return;
  }

  // 41257 marches its FULL 512x512 like a 41256 (owner-confirmed): A8 is only unusable in
  // NIBBLE mode, where the chip cycles it internally row- and column-wise. write16Pin and
  // read16Pin are single accesses -- one CAS toggle per RAS cycle -- so A8 addresses
  // normally there, and checkAddressShorts already drives it (PORTC mask 0x11 for this
  // type). Only the pattern and retention paths stay nibble-based at 256x256.
  marchAxes(max_rows, max_cols, &march16Pin);
}
...
void __attribute__((hot)) writeRow_16Pin(uint16_t row, uint16_t cols, uint8_t patNr) {
  ...
  if (is_nibble) {
    ...
    // write() runtime (delays[1]=0); delayRows effectively 1.
    ...
    // All other 16-pin types: shared retentionTail (common.cpp) — delays[1] + delayRows
...
void __attribute__((hot)) checkRow_16Pin(uint16_t cols, uint16_t row, uint8_t patNr, uint8_t check) {
  ...
  if (!is_nibble) {
    ...
    for (uint16_t col = 0; col < cols / 2; col++) {
      ...
      RAS_LOW16;
      setAddr16_Random(col);
      // KNOWN, deliberately NOT fixed: these four sample 31-94 ns after CAS↓, one cycle
      // earlier than the validated fastPatternNibble_16Pin (94-156 ns). Adding a NOP each
      // costs 4 cycles inside the RAS window, and the 41257 measures max 10.00 us against
      // its 10 us tRAS limit — zero headroom. A certain tRAS violation is worse than a
      // marginal tCAC on a part that measures clean. Revisit if the row loop gets faster.
      CAS_LOW16;
      CAS_HIGH16;
      data0 = PINC ^ data0;
      CAS_LOW16;
  ...
  if (row_errors) {
    ...
    if (CFG_32K_ACTIVE && (type == T_4164 || type == T_4164_2MS)) {
      // 5.0.8 MED-1: a retention fault in a quadrant that was structurally GOOD in the
      // checkerboard (bit set in newQuad but NOT in quadrantErrorsAfterChecker) takes the
      // 4ms->2ms retry instead of mislabelling a non-uniformly-decaying full 4164 as a
      // half-good 32K part; a real 4532/3732 fails only in its already-dead half (subset of
      // quadrantErrorsAfterChecker) -> deferred/classified at 4ms exactly as before.
      uint8_t newQuad = row_errors << ((row >= 128) ? 2 : 0);
      quadrantErrors |= newQuad;
      // 5.0.11: the guard used to be conditioned on retryActive, so it only ever ran
      // during iteration 1's 4 ms pass — during the 2 ms retry (retryActive=0) and in
      // every loop iteration after the first (pat45_done clears it) a retention fault in
      // a checker-GOOD quadrant was silently absorbed and the chip reported as a good
      // half-good 32K part. The condition never needed retryActive: defer only when the
      // failing quadrant was already dead in the checkerboard.
      if (!(newQuad & ~quadrantErrorsAfterChecker) && quadrantErrors != 0x0F)
        return;
    }
    if (retryActive) {
      retryActive = 2;
